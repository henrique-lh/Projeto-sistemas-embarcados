...
esp_err_t get_imu_data(IMUData *data)
{
	...
	while(1) {
		...
		ESP_ERROR_CHECK(i2c_master_read(cmd, data->sensorByteData, sizeof(data->sensorByteData), 1));
		ESP_ERROR_CHECK(i2c_master_stop(cmd));
		ESP_ERROR_CHECK(i2c_master_cmd_begin(I2C_NUM_0, cmd, 1000/portTICK_PERIOD_MS));
		i2c_cmd_link_delete(cmd);
		...
		ESP_ERROR_CHECK(i2c_master_start(cmd));
		ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (I2C_ADDRESS << 1) | I2C_MASTER_READ, 1));
		ESP_ERROR_CHECK(i2c_master_read(cmd, data->sensorByteData+8, sizeof(data->sensorByteData)-8, 1));
		ESP_ERROR_CHECK(i2c_master_stop(cmd));
		ESP_ERROR_CHECK(i2c_master_cmd_begin(I2C_NUM_0, cmd, 1000/portTICK_PERIOD_MS));
		i2c_cmd_link_delete(cmd);
