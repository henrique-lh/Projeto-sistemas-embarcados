...
esp_err_t imu_init(uint8_t devAddr, gpio_num_t sda_pin, gpio_num_t scl_pin) {
  ...
  // Verifica a presença do MPU6050
  uint8_t who_am_i = 0;
...
esp_err_t imu_get_acceleration_data(AccelerationData *data) {
  ...
  // Começa a comunicação com o MPU6050
  i2c_master_start(cmd);
  ...
  // Converte os dados brutos para valores em m/s²
...
esp_err_t imu_get_gyroscope_data(GyroscopeData *data) {
   if (data == NULL) {
    return ESP_FAIL; // Verifica se o ponteiro é nulo
  }
  ...
  // Converte os dados brutos para valores em rad/s
